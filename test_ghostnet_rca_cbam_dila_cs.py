...
def plot_calibration_curve(all_labels, all_probs):
    ...
    plt.savefig('/mnt/newdisk/KJY/多模态/Cancer_Embolus/ghostnet_rca_cbam_dila_cs/result/Calibration_Curve_test.png', dpi=1000)
    plt.close()
...
def plot_dca_curve(all_labels, all_probs):
    ...
    plt.grid(True)
    plt.savefig('/mnt/newdisk/KJY/多模态/Cancer_Embolus/ghostnet_rca_cbam_dila_cs/result/DCA_Curve_test.png', dpi=1000)
    plt.close()
...
class ClassificationDataset(Dataset):
    def __init__(self, img_dir,num_patches=100, transform=None):
        ...
        self.df=pd.read_excel('../selected_features_test.xlsx')
...
def evaluate_model(model, device, dataloader, criterion, adjustment_ratio=0.65):
    model.eval()
    loss = 0.0
    correct = 0
    total = 0
...
def plot_roc(all_labels, all_probs, num_classes):
    ...
    plt.savefig('/mnt/newdisk/KJY/多模态/Cancer_Embolus/ghostnet_rca_cbam_dila_cs/result/ROC_Curve_test.png', dpi=1000)

if __name__ == "__main__":
    # File paths for data
    val_data_fp = '/mnt/newdisk/KJY/RW39/data/data/外部_Cancer_Embolus'
    ...
    plt.savefig('/mnt/newdisk/KJY/多模态/Cancer_Embolus/ghostnet_rca_cbam_dila_cs/result/Confusion_Matrix_test.png', dpi=1000)
